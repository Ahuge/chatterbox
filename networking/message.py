from chatterbox.utils import MessageType, InvalidMessageFormatError, InvalidMessageVersionError
...
class StartChatServerMessage(Message):
    def __init__(self, user, hostname):
        super(StartChatServerMessage, self).__init__(
            user,
            hostname,
            message_type=MessageType.start_chat_server,
            message="",
        )


class StartChatClientMessage(Message):
    def __init__(self, user, hostname, port):
        super(StartChatClientMessage, self).__init__(
            user,
            hostname,
            message_type=MessageType.start_chat_client,
            message="%s" % port,
        )
